...
TELEGRAM_CONFIG_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'telegram_config.json')
...
def load_telegram_credentials():
    """Load Telegram credentials from new config first, then fallback to legacy config."""
    # New config generated by Telegram settings dialog
    if os.path.exists(TELEGRAM_CONFIG_PATH):
        try:
            with open(TELEGRAM_CONFIG_PATH, 'r', encoding='utf-8') as f:
                tg_config = json.load(f)
            token = str(tg_config.get('bot_token', '')).strip()
            chat_id = str(tg_config.get('chat_id', '')).strip()
            if token and chat_id and "YOUR_" not in token:
                return token, chat_id, "telegram_config.json"
            print(f"[Notifier] Telegram config exists but is incomplete: {TELEGRAM_CONFIG_PATH}")
        except Exception as e:
            print(f"[Notifier] Failed to read Telegram config {TELEGRAM_CONFIG_PATH}: {e}")
    ...
    token = str(legacy_config.get('telegram_bot_token', '')).strip()
    chat_id = str(legacy_config.get('telegram_chat_id', '')).strip()
    if token and chat_id and "YOUR_" not in token:
        return token, chat_id, "navigation/config.json"
